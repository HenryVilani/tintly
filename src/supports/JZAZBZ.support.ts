...
export interface JzAzBz extends BaseColor {
	type: "JZAZBZ";
	jz: number;
	az: number;
	bz: number;
...
export const JzAzBzSupport: ColorModel<JzAzBz> = {
	type: "JZAZBZ",
	...
	parse(input) {
		const m = input.match(
			/^jzazbz\(\s*([\d.]+)\s*,\s*([-\d.]+)\s*,\s*([-\d.]+)(?:\s*,\s*([\d.]+))?\s*\)$/i,
		);
		if (!m) return null;
		return {
			type: "JZAZBZ",
	...
	toCanonical(color) {
		const [x, y, z] = jzazbzToXYZ(color.jz, color.az, color.bz);
		return { x, y, z, alpha: color.alpha };
	},

	fromCanonical(color) {
		const [jz, az, bz] = xyzToJzazbz(color.x, color.y, color.z);
		return { type: "JZAZBZ", jz, az, bz, alpha: color.alpha };
	},

	toString(color) {
		const jz = Number(color.jz.toFixed(6));
		const az = Number(color.az.toFixed(6));
		const bz = Number(color.bz.toFixed(6));
		if (color.alpha === 1) return `jzazbz(${jz} ${az} ${bz})`;
		return `jzazbz(${jz} ${az} ${bz} / ${color.alpha})`;
	},
};


Tintly.register(JzAzBzSupport);

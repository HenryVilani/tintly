...
export interface Okhsv extends BaseColor {
	type: "OKHSV";
	h: number;
	s: number;
	v: number;
...
export const OkhsvSupport: ColorModel<Okhsv> = {
	type: "OKHSV",

	parse(input) {
		const m = input.match(
			/^okhsv\(\s*([\d.]+)\s*,\s*([\d.]+)%?\s*,\s*([\d.]+)%?(?:\s*,\s*([\d.]+))?\s*\)$/i,
		);
		if (!m) return null;
		return {
			type: "OKHSV",
	...
	toCanonical(color) {
		...
		const [Lc, Cc] = findCuspLS(a_, b_);
		const [S0, T] = getStMax(a_, b_, [Lc, Cc]);
		...
		const [Lmid, Cmid] = [Lc * 0.5, Cc * 0.5];
		const t = L / Lv;

		const [x, y, z] = oklabToXYZ(L, C * a_, C * b_);
		return { x, y, z, alpha: color.alpha };
	},

	fromCanonical(color) {
		const [L, a, b] = xyzToOklab(color.x, color.y, color.z);
		...
		const t = L / (L + C * (T - (T * k * L) / (S0 * Lc)));
		const Lv = t * L + (1 - t) * Lc;
		const Cv = t * C + (1 - t) * Cc;

		const v = L / Lv;
		const s = C > 1e-8 ? (Cv * S0 + T) / (T * (Cv + S0)) : 0;
	...
	toString(color) {
		...
		if (color.alpha === 1) return `okhsv(${h} ${s}% ${v}%)`;
		return `okhsv(${h} ${s}% ${v}% / ${color.alpha})`;
	},
};

Tintly.register(OkhsvSupport);
